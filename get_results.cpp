...
int main ()
{
  ...
  for (auto const& dir_entry : fs::directory_iterator(dir_path))
  {
    ...
    result_file << "No,Size,Mean,StdDev,CV,Chi2,Chi2 df\n";
...
std::pair<double,int> get_chi2 ( std::vector<unsigned int>& vec )
{
  ...
  int bin_number = std::ceil((max_val - min_val) / Scott_width); // Необходимое по правилу Скотта количество промежутков
